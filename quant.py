...
# This file was copied from here; https://gist.github.com/TheBloke/b47c50a70dd4fe653f64a12928286682
# It is included here for convenience on fine tune, merge and qauntize.
...
class QuantAutoGPTQ:
    def __init__(self, model_name_or_path, output_dir, dataset,
        ...
        print(f"Trying to load tokenizer from: {self.pretrained_model_dir}")
        if os.path.isdir(self.pretrained_model_dir):
            print("The path is a directory. Listing its contents:")
            print(os.listdir(self.pretrained_model_dir))
        else:
            print(f"The path {self.pretrained_model_dir} is not a directory or doesn't exist.")
    ...
    def quantize(self, output_dir, traindataset, bits, group_size, desc_act, damp):
        ...
        self.logger.info(
            f"Loading model from {self.pretrained_model_dir} with trust_remote_code={self.trust_remote_code} and dtype={torch_dtype}")
        ...
        model.quantize(traindataset, use_triton=self.use_triton, batch_size=self.batch_size,
                       cache_examples_on_gpu=self.cache_examples)
        ...
        try:
            if self.make_folder:
                output_dir = os.path.join(self.output_dir_base,
                                          f"{bits}bits-{group_size}g-desc_act_{desc_act}-damp_{damp}")
            else:
